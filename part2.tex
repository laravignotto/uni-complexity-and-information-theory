...
\subparagraph{URM} Si ha una URM con $x$ nel registro $R_0$. È sufficiente una singola istruzione $S(0)$, quindi la complessità è $\Theta(1)$.
...
\begin{definition}[Macchina di Turing a $k$-nastri]
    ...
    $$
        \delta : K\times\Sigma^k \to (K\cup\{\text{yes},\text{no},\text{halt}\}) \times
                                    (\Sigma \times \{\gets,\to,-\})^k
    $$    
\end{definition}
...
$$
con $\Sigma_h$ alfabeto di $\mathcal{M}_h$ ($|\Sigma_h|^i$ è il numero di input di lunghezza $i$ per $\mathcal{M}_h$). $N(i)$ è un numero molto grande. Ad esempio, $|x|=i$ input per $\mathcal{M}_3$, $\mathcal{M}_3\downarrow$.
...
\end{eqnarray*}
con $+2$ per yes e no, e $k$ numero di nastri della macchina. I \textcolor{RoyalBlue}{termini che dipendono dalla macchina} sono \textcolor{RoyalBlue}{$|x|+2$} e \textcolor{RoyalBlue}{$f(|x|)$}. Quindi
\begin{eqnarray*}
...
\begin{corollary}
    $$
        \text{N}\mathbb{L} \subseteq \text{SPACE}\left( (\log n)^2 \right)
    $$
\end{corollary}
