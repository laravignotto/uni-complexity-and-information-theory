...
\paragraph{Quantità di informazione di un evento} Proviamo a definire la quantità di informazione di un evento $h(E)$ come
$$
    h(E) = \frac{1}{p(E)}
$$
...
Se, invece, si ha una moneta truccata, con $P(H)=0.9$ e $P(T)=0.1$, la quantità di informazione media è
$$
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        ymax=.8,
        xtick={0,0.1,0.2,0.3},
        ytick={0,0.33219280948,0.46438561897,0.52108967825},
        xticklabels={0,$x_1$,\textcolor{ForestGreen}{$x$},$x_2$},
        yticklabels={0,$f(x_1)$,\textcolor{ForestGreen}{$f(x)$},$f(x_2)$},
        width=8cm,
        height=6cm,
        ]
        \addplot[domain=-0.5:4, samples=100, RoyalBlue, thick] {-x*log2(x)};
        % vertical lines
        \draw [dashed] (axis cs:0.1,0) -- (axis cs:0.1,0.33219280948);
        \draw [dashed] (axis cs:0.3,0) -- (axis cs:0.3,0.52108967825);
        \draw [dashed,ForestGreen] (axis cs:0.2,0) -- (axis cs:0.2,0.46438561897);
        % horizontal lines
        \draw [dashed] (axis cs:0,0.33219280948) -- (axis cs:0.1,0.33219280948);
        \draw [dashed] (axis cs:0,0.52108967825) -- (axis cs:0.3,0.52108967825);
        \draw [dashed,ForestGreen] (axis cs:0,0.46438561897) -- (axis cs:0.2,0.46438561897);
    \end{axis}
\end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[domain=0.01:0.99, samples=100, BlueViolet, thick] {-x*log2(x)-(1-x)*log2(1-x)};
            \draw [dashed,OliveGreen] (axis cs:0.5,0) -- (axis cs:0.5,1);
        \end{axis}
    \end{tikzpicture}
\end{center}
...
\begin{theorem}[Disuguaglianza di Kraft-McMillan, o Teorema Inverso]
    $$
        \varphi \text{ è UD}
...
% LEZIONE 5
\subsection{Quanto Possiamo Comprimere?}
...
Consideriamo $\varphi_S$ su $\mathcal{A}^n$ (tuple di lunghezza $n$).
$$
...
\begin{definition}[Lunghezza media per i caratteri di un codice, ELFC]
...
\begin{definition}[Codifica Ottimale]
...
\paragraph{Esempio} $\mathcal{A}=\{a,b,c,d,e,f\}$, $P=\{\frac{40}{100},\frac{18}{100},\frac{15}{100},\frac{13}{100},\frac{10}{100},\frac{4}{100}\}$. Come prima divisione si avrà
$$
    \left\{
        ...
        child {
            ...
            child {
                ...
                child {
                    ...
                    edge from parent[draw=RoyalBlue]
                    node[below] {$1$}
                }
                child {
                    node {$d$}
...
\paragraph{Esempio} $\mathcal{A}=\{a,b,c,d,e,f\}$, $P=\{\frac{40}{100},\frac{18}{100},\frac{15}{100},\frac{13}{100},\frac{10}{100},\frac{4}{100}\}$. Si parte dalle foglie $e$ e $f$, con probabilità $\frac{10}{100}$ e $\frac{4}{100}$, rispettivamente. Si uniscono in un nodo interno, con probabilità $\frac{14}{100}$. Si continua così fino ad arrivare alla radice.
\begin{center}
    \begin{tikzpicture}[grow=right,
    level 1/.style={sibling distance=2.5cm},
    ...
    \end{tikzpicture}
\end{center}
La EL in questo caso è $2.36$.\medskip

Il codice di Huffman è sempre ottimale. L'idea della dimostrazione (dimostrazione completa nel libro alla sezione 5.5) è che se si dimostra che i due caratteri con la codifica più lunga hanno la stessa lunghezza, si può dimostrare che il codice di Huffman è ottimale.
...
\subsection{Divergenza e Disuguaglianza di Gibbs}
...
\paragraph{Entropia Congiunta} % \paragraph{$\bm{X}$ e $\bm{Y}$ indipendenti} 
...
% \paragraph{$\bm{X}$ e $\bm{Y}$ dipendenti} 
